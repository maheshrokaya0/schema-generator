<script>
    ...
    let newCol, plusIcon, colItems, inputField, menu, menuItems, settingIcon, settings;
    ...
    function handleClick(event) {
        const clickedInsideMenu = menuItems.contains(event.target) || menu.contains(event.target);
            ...
            {#each newArray as item (item.id)}
                <div class="my-5">
                    <div class="flex justify-between items-center py-2 border rounded shadow-lg ">
                            ...
                            <i class="mx-2 hover:outline hover:outline-1 hover:outline-gray-200 hover:rounded-lg" bind:this={settingIcon}><button class="px-2 py-1 " on:click={()=> settingFun(item.id)}><Setting /></button></i>
                    </div>
                    <div class="hidden" bind:this={settings}>
                        <div class="flex justify-between items-center border py-2 shadow-md">
                            <div class="flex py-3 px-2">
                                <div class="bg-[#D9D9D9] px-3 py-2 rounded-lg font-semibold"><Button label="Not Empty" /></div>
                                <div class="ml-5 bg-[#D9D9D9] px-3 py-2 rounded-lg font-semibold"><Button label="Is Unique" /></div>
                            </div>
                            <div class="flex relative menus">
                                <div class="hidden right-10 absolute shadow-lg bg-white border px-3 py-2 rounded" bind:this={menuItems}>
                                    <p class="w-full border-b px-2 py-1 hover:bg-gray-200 hover:cursor-pointer"><button on:click={()=> menuFun()} >Delete</button></p>
                                    <p class="w-full px-2 py-1 hover:bg-gray-200 hover:cursor-pointer"><button on:click={()=> menuFun()} >Duplicate</button></p>
                                </div>
                                <button class="hover:cursor-pointer hover:bg-gray-200 rounded-full mr-3 py-1 px-2" bind:this={menu} on:click={()=> menuFun()} ><Menu /></button>
                            </div>
                        </div>
                    </div>
                </div>
