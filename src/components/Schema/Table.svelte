<script>
    ...
    const item = newArray.find(item => item.id === itemId);
      ...
      if (!item.toggle) {
        ...
        item.settingIcon.classList.add("bg-gray-200", "rounded-lg");
      } else {
        item.settings.classList.add("hidden");
        item.settingIcon.classList.remove("bg-gray-200", "rounded-lg");
      }
    }
  </script>
          ...
          {#if !item.hidden}
            <div class="my-5">
              <div class="flex justify-between items-center py-2 border rounded shadow-lg">
                ...
                <i class="mx-2 hover:outline hover:outline-1 hover:outline-gray-200 hover:rounded-lg" bind:this={item.settingIcon}>
